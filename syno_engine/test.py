...
if __name__ == '__main__':
    ...
    from model.t2g import NaiveT2G
    from utils.text import t2g_clean_text
    from utils.load import load_graph_neo4j

    print('loading...')
    model = NaiveT2G(ner=SpacyNER(), rext=OpenRE(), coref=AllenCOREF(), exclude_ents=[
                        'LANGUAGE', 'DATE', 'TIME', 'PERCENT', 'MONEY', 'QUANTITY', 'CARDINAL', 'ORDINAL']).load_model()

    cln_text = ". ".join(t2g_clean_text(text))
    graph = model(cln_text)
    for g in graph:
        print(g['sentence'])
        for r in g['relations']:
            if r['score'] > 0.5:
                print('\t', r['head']['name'], '->', r['relation'],
                        '->', r['tail']['name'], r['score'])

    url = "bolt://192.168.99.100:7687/"
    auth = ("neo4j", "test1234")
    load_graph_neo4j(graph, url, auth)

    """
    MATCH (h)-[p]->(t)
    where p.score > 0.5 and p.score is not null
    RETURN h, p, t
    """